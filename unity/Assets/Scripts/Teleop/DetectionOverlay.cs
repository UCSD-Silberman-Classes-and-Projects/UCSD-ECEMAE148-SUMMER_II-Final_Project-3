...
namespace Teleop
{
    ...
    public class DetectionOverlay : MonoBehaviour
    {
        ...
        [Header("Selection")]
        [Range(1f, 3f)]
        [Tooltip("Collider padding multiplier. 1 = exact box, 2 = double size. Increase if hard to select.")]
        public float colliderPadding = 1.5f;

        [Header("Selection")]
        [Range(0.3f, 1.5f)]
        [Tooltip("Selection sphere radius as fraction of the larger box dimension.")]
        ...
        private class BoxView
        {
            ...
            public LineRenderer corners;
            public SphereCollider collider;   // ← change BoxCollider to SphereCollider
        ...
        private void Render(BoxView view, Detection det, Rect rect, bool isSelected)
        {
            ...
            // Replace the collider sizing block with:
            float worldW = rect.width / Mathf.Max(1f, surface.FrameWidth) * surface.SurfaceWidth;
            float worldH = rect.height / Mathf.Max(1f, surface.FrameHeight) * surface.SurfaceHeight;
            float radius = Mathf.Max(worldW, worldH) * 0.6f;  // covers the box generously

            view.collider.transform.position = (tl + br) * 0.5f;
            view.collider.transform.rotation = surface.transform.rotation;
            view.collider.radius = radius;
        ...
        private static Material MakeOverlayMat()
        {
            // Try URP unlit first, fall back to built-in
            var shader = Shader.Find("Universal Render Pipeline/Unlit")
                      ?? Shader.Find("Unlit/Color");
            var mat = new Material(shader);
            // Always draw on top — ignore depth buffer
            mat.SetInt("_ZWrite", 0);
            mat.SetInt("_ZTest", (int)UnityEngine.Rendering.CompareFunction.Always);
            mat.renderQueue = 4000; // overlay queue, above everything
            return mat;
        }
        private BoxView CreateView(int index)
        {
            ...
            var rb = colliderObj.AddComponent<Rigidbody>();
            rb.isKinematic = true;      // we move it manually, not physics
            rb.useGravity = false;
            ...
            return new BoxView
            {
                ...
                collider = sphere, label = text, marker = marker
            };
        }
    }
}
