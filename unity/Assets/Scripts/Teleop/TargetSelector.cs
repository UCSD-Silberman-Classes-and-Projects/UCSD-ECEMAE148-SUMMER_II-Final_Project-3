...
namespace Teleop
{
    ...
    public class TargetSelector : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Use right controller anchor if rayOrigin not set in Inspector
            if (rayOrigin == null)
            {
        ...
        private void Update()
        {
            ...
            {
                _hovered = target;
                // Brighten ray when hovering a box
                var c = target != null
                    ? rayColor
                    : new Color(rayColor.r, rayColor.g, rayColor.b, 0.2f);
            ...
            if (ClearPressed()) hub.ClearSelection();
            if (StopPressed()) hub.Stop();
            if (FollowPressed()) hub.RequestMode("follow");
            if (AvoidPressed()) hub.RequestMode("avoid");
        }
        ...
        private DetectionBox Probe(Ray ray, out float distance)
        {
            ...
            int count = Physics.RaycastNonAlloc(ray, _hits, maxDistance, ~0,
                                    QueryTriggerInteraction.Collide);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                Debug.Log($"Hit: {_hits[i].collider.name} box={box} trackId={box?.trackId}"); // ← add
        ...
        protected virtual bool SelectPressed() => OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger, OVRInput.Controller.RTouch);

        protected virtual bool ClearPressed() => OVRInput.GetDown(OVRInput.Button.One, OVRInput.Controller.LTouch);  // X button

        protected virtual bool StopPressed() => OVRInput.GetDown(OVRInput.Button.PrimaryThumbstick, OVRInput.Controller.All);

        protected virtual bool FollowPressed() => OVRInput.GetDown(OVRInput.Button.One, OVRInput.Controller.RTouch);  // A button

        protected virtual bool AvoidPressed() => OVRInput.GetDown(OVRInput.Button.Two, OVRInput.Controller.RTouch);  // B button
    }
}
