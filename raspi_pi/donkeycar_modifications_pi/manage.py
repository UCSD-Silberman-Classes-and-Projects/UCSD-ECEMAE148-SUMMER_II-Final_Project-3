...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    start_server()   # starts WebSocket listener in background thread
...
#     #
#     # setup primary camera
#     # COMMENTED OUT: replaced by OakCamera inside add_custom_pilot()
#     #
#     # add_camera(V, cfg, camera_type)
...
#     # ── COMMENTED OUT: Keras/TF model inference block ────────────────────────
#     # Replaced by OakCamera + LanePilot + YoloSignPilot + CommandSelector.
#     # CommandSelector is now the sole writer of pilot/steering and pilot/throttle.
#     #
#     # def load_model(kl, model_path):
#     #     start = time.time()
#     #     print('loading model', model_path)
...
#     # ── Custom OAK-D + Lane + YOLO + CommandSelector pipeline ────────────────
#     # OakCamera replaces add_camera(); CommandSelector owns pilot/steering
#     # and pilot/throttle. Everything downstream (DriveMode, VESC) is untouched.
#     # Inlined from manage_patch.py — no external file dependency.
#     from new_hackathon.oak_camera       import OakCamera
#     from new_hackathon.lane_pilot       import LanePilot
#     from new_hackathon.yolo_sign_pilot  import YoloSignPilot
#     from new_hackathon.command_selector import CommandSelector

#     # 1. OAK-D Camera with color correction
#     cam = OakCamera(
#         resolution=(416, 416),
#         fps=20,
...
#     # 2. Lane Pilot — CV-based right-lane keeping, outputs steering only
#     lane = LanePilot(
#         image_size=(416, 216),
#         roi_top_frac=0.5,
#         kp=0.8,
...
#         show_vis=True,
#     )
#     V.add(lane, inputs=['cam/image_array'], outputs=['lane/steering'],
#           threaded=True, run_condition='run_pilot')

#     # 3. YOLO Sign Pilot — runs hackathon_best_v3.pt (30 classes)
#     yolo = YoloSignPilot(
#         model_path='hackathon_best_v3.pt',
#         confidence=0.25,
#         input_size=416,
#         debug=True,
#     )
#     V.add(yolo, inputs=['cam/image_array'],
#           outputs=['sign/active', 'sign/steering', 'sign/throttle'],
#           run_condition='run_pilot')

#     # 4. Command Selector — sole final authority on pilot/steering + pilot/throttle
#     sel = CommandSelector(
#         cruise_throttle=0.10,
#         debug=True,
#     )
#     V.add(sel,
#           inputs=['sign/active', 'sign/steering', 'sign/throttle', 'lane/steering'],
#           outputs=['pilot/steering', 'pilot/throttle'],
#           run_condition='run_pilot')

#     # DriveMode reads 'pilot/angle' but CommandSelector outputs 'pilot/steering'.
#     # This pipe aliases them so DriveMode receives the correct value.
#     V.add(Pipe(), inputs=['pilot/steering'], outputs=['pilot/angle'])
#     # ── END custom pipeline ───────────────────────────────────────────────────
...
#                     throttle_scale=cfg.JOYSTICK_MAX_THROTTLE,
#                     steering_scale=cfg.JOYSTICK_STEERING_SCALE,
#                     auto_record_on_throttle=cfg.AUTO_RECORD_ON_THROTTIC)
#                 ctr.set_deadzone(cfg.JOYSTICK_DEADZONE)
#             elif cfg.CONTROLLER_TYPE == "MM1":
#                 from donkeycar.parts.robohat import RoboHATController
#                 ctr = RoboHATController(cfg)
...
#     """
#     Add the configured camera to the vehicle pipeline.
#     NOTE: This function is kept intact but is NOT called in drive().
#           OakCamera (via add_custom_pilot) replaces it entirely.
